...
SPORTS = ("running", "cycling", "swimming", "multisport", "strength", "mobility", "walking", "transition", "other")
...
def _history_sport(act):
    ...
    if any(x in raw for x in ("strength", "weight_training", "weighttraining", "cardio_strength", "gym", "functional_strength")): return "strength"
    if any(x in raw for x in ("mobility", "stretch", "flexibility", "yoga", "pilates")): return "mobility"
    if any(x in raw for x in ("walk", "walking", "hike", "hiking")): return "walking"
    ...
    if any(x in raw for x in ("multi", "triathlon", "duathlon", "aquathlon")): return "multisport"
    return "other"
...
def _history_finalize(bucket):
    ...
    if bucket.get("exercise_load_available"): out["exercise_load"] = round(bucket["exercise_load"], 1)
    return out
...
def _merge(buckets):
    ...
    for bucket in buckets:
        total["activity_count"] += bucket["activity_count"]; total["distance_km"] += bucket["distance_km"]; total["duration_hours"] += bucket["duration_hours"]
        if bucket.get("exercise_load_available"): total["exercise_load"] += bucket["exercise_load"]; total["exercise_load_available"] = True
    return total


def get_training_history(api, target_date):
    start_history_date = target_date - timedelta(days=27)
    try: historical_activities = api.get_activities_by_date(start_history_date.isoformat(), target_date.isoformat())
    except Exception: historical_activities = []
    historical_activities = generator._history_expand_multisport(api, historical_activities)
    by_date = {}
    for act in historical_activities: by_date.setdefault(generator._history_date(act, target_date), []).append(act)

    def window(start_date, end_date):
        buckets = {s: _history_empty() for s in SPORTS}
        for d, acts in by_date.items():
            if start_date <= d <= end_date:
                for act in acts: _history_add(buckets[_history_sport(act)], act)
        return buckets, _merge([buckets[s] for s in ENDURANCE_SPORTS]), _merge(list(buckets.values()))

    seven_start = target_date - timedelta(days=6)
    seven_sport, seven_endurance, seven_training = window(seven_start, target_date)
    weekly = []
    for i in range(4):
        week_end = target_date - timedelta(days=i * 7); week_start = week_end - timedelta(days=6)
        buckets, endurance, total_training = window(week_start, week_end)
        weekly.append({"week_start": week_start.isoformat(), "week_end": week_end.isoformat(), "total_endurance": _history_finalize(endurance), "total_training": _history_finalize(total_training), "sports": {s: _history_finalize(buckets[s]) for s in SPORTS if buckets[s]["activity_count"] > 0}})
    weekly.reverse()
    sport_7 = {s: _history_finalize(seven_sport[s]) for s in SPORTS if seven_sport[s]["activity_count"] > 0}
    total_7_endurance = _history_finalize(seven_endurance); total_7_training = _history_finalize(seven_training)
    running_weekly = [w["sports"].get("running", {}).get("distance_km", 0.0) for w in weekly]
    running_28_total = round(sum(running_weekly), 2); running_avg = round(running_28_total / len(running_weekly), 1) if running_weekly else 0.0
    running_7 = sport_7.get("running", {}).get("distance_km", 0.0)
    return {"7_day": {"total_endurance": total_7_endurance, "total_training": total_7_training, "sports": sport_7}, "28_day": {"avg_weekly_running_distance_km": running_avg, "total_running_distance_km": running_28_total, "weekly_total_endurance": weekly}, "running_volume": {"7_day_km": running_7, "28_day_km": running_28_total, "28_day_avg_weekly_km": running_avg}, "legacy_running_summary": {"7_day_distance_km": running_7, "28_day_avg_weekly_distance_km": running_avg, "weekly_distance_last_4_weeks_km": running_weekly}}
...
def _enrich_activity_environment(api, activity):
    ...
    def pick(keys):
        value = _first(summary, keys)
        return value if value is not None else _first(detail, keys)
