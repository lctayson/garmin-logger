...
def to_columnar(rows):
    if not isinstance(rows, list) or not rows or not all(isinstance(row, dict) for row in rows): return rows
    columns = []
    for row in rows:
        for key in row:
...
def compact_activity(activity):
    ...
    if split_key: activity[split_key] = to_columnar(activity[split_key])
    return activity
...
def _compact_data_arrays(text):
    ...
    while True:
        marker_pos = text.find(marker, search_from)
        if marker_pos < 0: break
        array_start = marker_pos + len('"data": '); depth = 0; in_string = False; escaped = False; array_end = None
        for i in range(array_start, len(text)):
            ch = text[i]
            if in_string:
                if escaped: escaped = False
            ...
            if ch == '"': in_string = True
                ...
                if depth == 0: array_end = i; break
        if array_end is None: break
        rows = json.loads(text[array_start:array_end + 1]); indent = len(text[:marker_pos].rsplit("\n", 1)[-1]); row_indent = " " * (indent + 2)
        row_text = "[\n" + ",\n".join(row_indent + json.dumps(row, ensure_ascii=False, separators=(", ", ": ")) for row in rows) + "\n" + " " * indent + "]"
        text = text[:array_start] + row_text + text[array_end + 1:]; search_from = array_start + len(row_text)
    return text

def _metrics_text(payload):
    lines = ["{"]; items = list(payload.items()); readable = {"date", "daily_readiness", "health_stats", "training_status"}
    for index, (key, value) in enumerate(items):
        comma = "," if index < len(items) - 1 else ""
        if key in readable:
            value_lines = json.dumps(value, ensure_ascii=False, indent=2).splitlines()
            lines.append(f'  {json.dumps(key, ensure_ascii=False)}: {value_lines[0]}'); lines.extend("  " + line for line in value_lines[1:])
...
def write_json(path, payload, activity_compact=False, metrics=False):
    tmp = f"{path}.tmp"; text = _metrics_text(payload) if metrics else json.dumps(payload, ensure_ascii=False, indent=2)
...
def refresh_latest_activities(data_dir, target_date, dated_activities, has_activities, today_date=None):
    """Refresh latest_activities only when the export is for today and has an activity."""
...
def main():
    ...
    payload = load_json(latest_path); target_date = args.date or payload.get("date")
