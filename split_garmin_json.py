...
def split_payload(payload):
    ...
    # Canonical metrics are built once, in memory. compact_metrics also handles
    # trends, duplicate Garmin representations, and future unknown top-level keys.
...
def _compact_data_arrays(text):
    # Trend/body-battery column headers stay on one line; each data row also
    # stays on one line. This gives compact files without making the tables hard to read.
...
def main():
    ...
    if payload.get("date") and payload["date"] != target_date.isoformat():
        raise ValueError(
            f"Date mismatch: --date={target_date.isoformat()} but JSON date={payload['date']}"
        )
