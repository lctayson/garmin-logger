...
def get_training_status_details(api, target_date_str):
    ...
    if recovery is not None: result['recovery_time_hours']=_safe_int(recovery)
    balance=raw.get('mostRecentTrainingLoadBalance',{}) or {}; bmap=balance.get('metricsTrainingLoadBalanceDTOMap',{}) or {}; be=next(iter(bmap.values()),{}) if isinstance(bmap,dict) else {}
    focus=be.get('trainingLoadBalanceFeedbackPhrase') or be.get('trainingBalanceFeedbackPhrase')
    if focus: result['load_focus']=humanize_enum(focus)
    monthly={
        'aerobic_low':_safe_float(be.get('monthlyLoadAerobicLow'),1),'aerobic_low_target_min':_safe_int(be.get('monthlyLoadAerobicLowTargetMin')),'aerobic_low_target_max':_safe_int(be.get('monthlyLoadAerobicLowTargetMax')),
        'aerobic_high':_safe_float(be.get('monthlyLoadAerobicHigh'),1),'aerobic_high_target_min':_safe_int(be.get('monthlyLoadAerobicHighTargetMin')),'aerobic_high_target_max':_safe_int(be.get('monthlyLoadAerobicHighTargetMax')),
        'anaerobic':_safe_float(be.get('monthlyLoadAnaerobic'),1),'anaerobic_target_min':_safe_int(be.get('monthlyLoadAnaerobicTargetMin')),'anaerobic_target_max':_safe_int(be.get('monthlyLoadAnaerobicTargetMax'))}
    monthly={k:v for k,v in monthly.items() if v is not None}
    if monthly: result['monthly_load_balance']=monthly
    vo2=raw.get('mostRecentVO2Max',{}) or {}; generic=vo2.get('generic',{}) if isinstance(vo2.get('generic'),dict) else {}
    value=_safe_float(_deep_get(generic,['vo2MaxValue','vo2MaxPreciseValue','vo2Max'])); status=generic.get('vo2MaxStatus') or generic.get('fitnessLevel') or generic.get('vo2MaxCategory') or generic.get('category') or _deep_get(vo2,['vo2MaxStatus','fitnessLevel'])
    if value is not None or status:
        result['vo2_max']={'value':value,'status':humanize_enum(status) if isinstance(status,str) else status}; result['vo2_max']={k:v for k,v in result['vo2_max'].items() if v is not None}
    heat=vo2.get('heatAltitudeAcclimation',{}) or raw.get('heatAltitudeAcclimation',{}) or {}; hp=heat.get('heatAcclimationPercentage'); ht=heat.get('heatTrend'); ap=heat.get('altitudeAcclimationPercentage') or heat.get('acclimationPercentage'); at=heat.get('altitudeTrend')
    if hp is not None or ht: result['heat_acclimation']={'percentage':_safe_int(hp),'trend':humanize_enum(ht) if isinstance(ht,str) else ht}
    if ap is not None or at: result['altitude_acclimation']={'percentage':_safe_int(ap),'trend':humanize_enum(at) if isinstance(at,str) else at}
    return {k:v for k,v in result.items() if v is not None}


def get_metric_trend(api,target_date,days=14,interval=1):
    from garmin_to_json import get_health_stats
    offsets=list(range(0,days,interval))
    if offsets and offsets[-1]!=days-1: offsets.append(days-1)
    trend=[]
    for offset in offsets:
        d=target_date-timedelta(days=days-1-offset); ds=d.isoformat()
        try: health=get_health_stats(api,ds)
        except Exception: health={}
        try: status=get_training_status_details(api,ds)
        except Exception: status={}
        entry={'date':ds,'resting_heart_rate':health.get('resting_heart_rate'),'hrv_last_night_avg_ms':_deep_get(health,['hrv.last_night_avg_ms']),'hrv_seven_day_avg_ms':_deep_get(health,['hrv.seven_day_avg_ms']),'total_sleep_hours':health.get('total_sleep_hours'),'sleep_score':health.get('sleep_score'),'vo2_max':_deep_get(status,['vo2_max.value']),'training_status':status.get('status'),'acute_load':_deep_get(status,['training_load.acute_load']),'chronic_load':_deep_get(status,['training_load.chronic_load']),'acwr':_deep_get(status,['training_load.acwr']),'acwr_status':_deep_get(status,['training_load.acwr_status'])}
...
def get_activities(api,target_date_str):
    ...
    for act in acts:
        ...
        obj={'activityId':aid,'name':act.get('activityName'),'type':typ,'distance_km':round(dist/1000,2) if dist else 0,'duration_mins':round(dur/60,2) if dur else 0,'avg_pace':format_pace(dist,dur,typ),'average_hr':_safe_float(act.get('averageHR') or act.get('avgHR')),'max_hr':_safe_float(act.get('maxHR') or act.get('maximumHR')),'aerobic_training_effect':_safe_float(act.get('aerobicTrainingEffect')),'anaerobic_training_effect':_safe_float(act.get('anaerobicTrainingEffect'))}
        if aid:
