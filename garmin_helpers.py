...
def _activity_load(activity):
    value = _deep_get(activity, ['activityTrainingLoad','trainingLoad','exerciseLoad','activityTrainingLoadDTO.trainingLoad','summaryDTO.trainingLoad','summaryDTO.exerciseLoad','activityTrainingLoadDTO.exerciseLoad'])
...
def _aggregate_activities(activities):
    ...
    for activity in activities:
        ...
        total['activity_count'] += 1; total['distance_km'] += dist; total['duration_hours'] += hours
        if load is not None:
            bucket['exercise_load'] += load; bucket['exercise_load_available'] = True
...
def get_metric_trend(api, target_date, days=14, interval=1):
    ...
    if interval == 1:
        ...
        while remaining>0:
            window_days=min(interval,remaining); start=end-timedelta(days=window_days-1); windows.append((start,end)); end=start-timedelta(days=1); remaining-=window_days
        windows.reverse()
    for start_date,end_date in windows:
        ...
        try: activities=api.get_activities_by_date(start_date.isoformat(),end_date.isoformat())
        except Exception as e: print(f'[trend_activities] Warning {start_date}..{end_date}: {e}',file=sys.stderr); activities=[]
        sports,total=_aggregate_activities(activities if isinstance(activities,list) else [])
        entry={'date':ds,'window_start':start_date.isoformat(),'window_end':end_date.isoformat(),'window_days':(end_date-start_date).days+1,'resting_heart_rate':health.get('resting_heart_rate'),'hrv_last_night_avg_ms':_deep_get(health,['hrv.last_night_avg_ms']),'hrv_seven_day_avg_ms':_deep_get(health,['hrv.seven_day_avg_ms']),'total_sleep_hours':health.get('total_sleep_hours'),'sleep_score':health.get('sleep_score'),'vo2_max':_deep_get(status,['vo2_max.value']),'training_status':status.get('status'),'acute_load':_deep_get(status,['training_load.acute_load']),'chronic_load':_deep_get(status,['training_load.chronic_load']),'acwr':_deep_get(status,['training_load.acwr']),'acwr_status':_deep_get(status,['training_load.acwr_status']),'activity_count':total['activity_count'],'total_endurance_duration_hours':total['duration_hours'],'total_endurance_distance_km':total['distance_km'],'sport_volume':{sport:{'activity_count':values['activity_count'],'distance_km':values['distance_km'],'duration_hours':values['duration_hours'],**({'exercise_load':values['exercise_load']} if 'exercise_load' in values else {})} for sport,values in sports.items() if values['activity_count']>0}}
        if total.get('exercise_load') is not None: entry['total_exercise_load']=total['exercise_load']
